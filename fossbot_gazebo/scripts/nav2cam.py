...
def main():
    print("=" * 60)
    print("STARTING FOSSBOT SIMULATION WITH NAV2 (CAMERA)")
    print("=" * 60)
    ...
    nav2cam_launch = os.path.join(inner_path, "launch", "nav2cam.py")
    maps_dir = os.path.join(inner_path, "maps", "rtabcam.yaml")
    nav2_params = os.path.join(inner_path, "params", "nav2cam.yaml")
    rviz_config = os.path.join(inner_path, "config", "nav2cam.rviz")
    ...
    if not os.path.exists(urdf_path):
        ...
        sys.exit(1)
    if not os.path.exists(nav2cam_launch):
        print(f"ERROR: NAV2+Camera launch file not found: {nav2cam_launch}")
        sys.exit(1)
    ...
    # 7. Εκκίνηση RTAB-Map με Camera για NAV2
    print("\n>>> Step 7: Starting RTAB-Map with Camera for NAV2...")
    rtab_cam_process = run_command(
        f'ros2 launch {nav2cam_launch} '
        f'use_sim_time:=true '
        f'localization:=true',
        background=True
    )
    print(f"  - RTAB-Map with Camera started with launch file: {nav2cam_launch}")
    time.sleep(10)
    
    # 8. Εκκίνηση Navigation2
    print("\n>>> Step 8: Starting Navigation2...")
    print("  - Using map: rtabcam.yaml")
    ...
    print("\n>>> Step 10: Publishing initial pose...")
    initial_pose_cmd = (
    ...
    # 11. Τελικές οδηγίες
    print("\n" + "=" * 60)
    print("✓ SIMULATION WITH NAV2 (CAMERA) STARTED SUCCESSFULLY")
    print("=" * 60)
    print(f"\nPackage: fossbot_gazebo")
    print(f"World: {world_path}")
    print(f"Map: {maps_dir}")
    print(f"NAV2 Params: {nav2_params}")
    print(f"Launch File: {nav2cam_launch}")
    ...
    try:
        ...
        processes = [ign_process, bridge_process, rtab_cam_process, nav2_process, rviz_process]
        if rsp_process:
