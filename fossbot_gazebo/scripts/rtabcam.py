...
def main():
    print("=" * 60)
    print("STARTING FOSSBOT SIMULATION WITH RTAB-MAP (CAMERA)")
    print("=" * 60)
    ...
    rtabcam_launch = os.path.join(inner_path, "launch", "rtabcam.py")
    rtabcam_rviz_config = os.path.join(inner_path, "config", "rtabcam.rviz")
    ...
    if not os.path.exists(urdf_path):
        ...
        sys.exit(1)
    if not os.path.exists(rtabcam_launch):
        print(f"ERROR: RTAB-Map camera launch file not found: {rtabcam_launch}")
        sys.exit(1)
    ...
    # 7. RTAB-Map (με κάμερα)
    print("\n>>> Step 7: Starting RTAB-Map with Camera and LiDAR...")
    rtabcam_process = run_command(
        f'ros2 launch {rtabcam_launch} use_sim_time:=true localization:=false',
        background=True
    )
    print(f"  - RTAB-Map started with launch file: {rtabcam_launch}")
    time.sleep(15)
    ...
    # 9. Εμφάνιση οδηγιών
    print("\n" + "=" * 60)
    print("✓ SIMULATION WITH RTAB-MAP (CAMERA) STARTED SUCCESSFULLY")
    print("=" * 60)
    print(f"\nPackage: fossbot_gazebo")
    print(f"World: {world_path}")
    print(f"RTAB-Map Launch: {rtabcam_launch}")
    ...
    print(f"  cp {maps_dir}/rtabcam.db {maps_dir}/rtabcam_map_{timestamp}.db")
    
    print("\nΓια μετατροπή του χάρτη για NAV2:")
    print(f"Μετά την ολοκλήρωση της χαρτογράφησης και το κλείσιμο εκτέλειτε την εντολή")
    print(f"  rtabmap-databaseViewer {maps_dir}/rtabcam.db")
    print(f"Και στον Viewer ακολουθείτε FILE->EXPORT OPTIMIZED 2D MAP")
    print(f"Αποθήκευση του χάρτη με το όνομα cam.pgm")
    print(f"ώστε να υπάρχει συμβατότητα με το πρόγραμμα nav2cam")
    
    print("\n" + "-" * 60)
    print("📌 INSTRUCTIONS FOR RTAB-MAP VISUALIZATION:")
    print("-" * 60)
    print(f"\n1. In RVIZ2, go to: File -> Open Config")
    print(f"2. Navigate to: {rtabcam_rviz_config}")
    print("3. Select rtabcam.rviz and click Open")
    ...
    try:
        ...
        processes = [ign_process, bridge_process, rtabcam_process, rviz_process]
        if rsp_process:
