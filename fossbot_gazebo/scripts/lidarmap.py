...
def main():
    print("=" * 60)
    print("STARTING FOSSBOT SIMULATION WITH SLAM_TOOLBOX (LIDAR MAPPING)")
    print("=" * 60)
    ...
    slam_params = os.path.join(inner_path, "params", "lidarmap.yaml")
    rviz_config = os.path.join(inner_path, "config", "lidarmap.rviz")
    ...
    if not os.path.exists(slam_params):
        print(f"ERROR: SLAM Toolbox params file not found: {slam_params}")
        sys.exit(1)
    ...
    # 7. Εμφάνιση οδηγιών τηλεχειρισμού
    print("\n" + "-" * 60)
    print("TELEOPERATION COMMANDS:")
    print("-" * 60)
    print("\nΓια τηλεχειρισμό εκτέλεσε την εντολή:")
    ...
    # 8. Εκκίνηση SLAM Toolbox
    print("\n>>> Step 7: Starting SLAM Toolbox for mapping...")
    print(f"  - Using params: {slam_params}")
    
    slam_process = run_command(
    ...
    # 9. Τερματισμός τυχόν υπάρχοντος RVIZ2 και εκκίνηση νέου
    print("\n>>> Step 8: Starting RVIZ2 for mapping...")
    run_command("pkill -f 'rviz2'", ignore_errors=True)
    time.sleep(1)
    ...
    # 10. Τελικές οδηγίες
    print("\n" + "=" * 60)
    print("✓ MAPPING WITH SLAM_TOOLBOX STARTED SUCCESSFULLY")
    print("=" * 60)
    print(f"\nPackage: fossbot_gazebo")
    print(f"World: {world_path}")
    print(f"SLAM Params: {slam_params}")
    ...
    print("\nΓια αποθήκευση του χάρτη εκτέλεσε σε άλλο τερματικό την εντολή:")
    print(f'  ros2 service call /slam_toolbox/save_map slam_toolbox/srv/SaveMap "{{name: {{data: \'{maps_dir}/lidarmap\'}}}}"')

    
    print("\nΓια διακοπή πατήστε Ctrl+C")
    
    # 11. Διατήρηση processes
    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        print("\n\nShutting down mapping...")
        processes = [ign_process, bridge_process, slam_process, rviz_process]
        if rsp_process:
