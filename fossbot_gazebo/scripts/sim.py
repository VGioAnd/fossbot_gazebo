...
def main():
    print("=" * 60)
    print("STARTING FOSSBOT SIMULATION")
    print("=" * 60)
    
    package_path = get_package_path()
    model_path = os.path.join(package_path, "fossbot_gazebo", "models", "fossbot")
    world_path = os.path.join(package_path, "fossbot_gazebo", "worlds", "obstacles.world")
    urdf_path = os.path.join(model_path, "model.urdf")
    sdf_path = os.path.join(model_path, "model.sdf")
    rviz_config = os.path.join(package_path, "fossbot_gazebo", "config", "sim.rviz")
    ...
    try:
        ...
        rsp_process = subprocess.Popen(cmd, 
                                       stdout=subprocess.DEVNULL, 
                                       stderr=subprocess.DEVNULL)
        print(f"  - Robot State Publisher started successfully")
    except Exception as e:
        print(f"  - Error starting Robot State Publisher: {e}")
        rsp_process = None
    
    # 7. RVIZ2
    print("\n>>> Step 7: Starting RVIZ2...")
    ...
    # 9. Διατήρηση processes
    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        print("\n\nShutting down simulation...")
        processes = [ign_process, bridge_process, rviz_process]
        if rsp_process:
